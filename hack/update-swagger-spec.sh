...
kube::log::status "Starting fledged"
export KUBERNETES_SERVICE_HOST=$API_HOST
export KUBERNETES_SERVICE_PORT=$API_PORT

sleep 5s
kubectl apply -f deploy/fledged-crd.yaml --server=http://127.0.0.1:8050
sleep 2s
kubectl apply -f deploy/fledged-namespace.yaml --server=http://127.0.0.1:8050
sleep 2s
kubectl apply -f deploy/fledged-serviceaccount.yaml --server=http://127.0.0.1:8050
sleep 2s
kubectl apply -f deploy/fledged-clusterrole.yaml --server=http://127.0.0.1:8050
sleep 2s
kubectl apply -f deploy/fledged-clusterrolebinding.yaml --server=http://127.0.0.1:8050
sleep 2s
kubectl apply -f deploy/fledged-deployment-swagger.yaml --server=http://127.0.0.1:8050
sleep 20000s
